...
static const char *postFragShader = 
    ...
    "    float paperMix = smoothstep(0.20, 0.80, lum) * 0.35;\n"
    ...
    "    \n"
    "    result = mix(result, inkColor, inkMask * 0.55);\n"
    "    \n"
    "    float h1 = step(0.5, fract((uv.x + uv.y) * 26.0));\n"
    "    float h2 = step(0.5, fract((uv.x - uv.y) * 26.0));\n"
    "    float hatch = 0.0;\n"
    "    if (lum > 0.38 && lum < 0.82) hatch = max(h1 * 0.65, h2 * 0.45);\n"
    "    result = mix(result, inkColor * 0.75, hatch * 0.35);\n"
    "    \n"
