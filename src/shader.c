...
static const char *postFragShader = 
    ...
    "    float paperMix = smoothstep(0.15, 0.85, lum) * 0.70;\n"
    "    vec3 result = mix(col, paper, paperMix);\n"
    "    \n"
    "    float darkLift = smoothstep(0.0, 0.15, lum);\n"
    "    result = mix(paper * 0.95, result, darkLift);\n"
    "    \n"
    "    result = mix(result, inkColor, inkMask * 0.65);\n"
    "    \n"
    "    float h1 = step(0.5, fract((uv.x + uv.y) * 32.0));\n"
    "    float h2 = step(0.5, fract((uv.x - uv.y) * 32.0));\n"
    "    float hatch = 0.0;\n"
    "    if (lum > 0.35 && lum < 0.85) hatch = max(h1 * 0.85, h2 * 0.65);\n"
    "    result = mix(result, inkColor * 0.85, hatch * 0.55);\n"
    "    \n"
