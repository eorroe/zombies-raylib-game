...
static const char *postFragShader = 
    ...
    "    float paperMix = smoothstep(0.20, 0.80, lum) * 0.20;\n"
    ...
    "    float h3 = step(0.5, fract(uv.x * 32.0));\n"
    "    float hatch = 0.0;\n"
    "    if (lum > 0.25 && lum < 0.95) hatch = max(h1 * 0.85, max(h2 * 0.65, h3 * 0.45));\n"
    "    result = mix(result, inkColor * 0.85, hatch * 0.50);\n"
    "    \n"
