...
static const char *postFragShader = 
    ...
    "    float paperMix = smoothstep(0.15, 0.85, lum) * 0.75;\n"
    ...
    "    float hatch = 0.0;\n"
    "    if (lum > 0.28 && lum < 0.88) hatch = max(h1 * 0.85, h2 * 0.65);\n"
    "    result = mix(result, inkColor * 0.85, hatch * 0.25);\n"
    "    \n"
    ...
    "    result = mix(result, inkBlue * 0.8, marginL * 0.70);\n"
    "    \n"
