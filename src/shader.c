...
static const char *postFragShader = 
    ...
    "    edge = smoothstep(0.12, 0.40, edge);\n"
    "    float inkMask = floor(edge * 5.0 + 0.5) / 5.0;\n"
    "    \n"
    "    vec3 paper = mix(vec3(0.99, 0.96, 0.90), vec3(0.96, 0.93, 0.86), uv.y);\n"
    ...
    "    float paperMix = smoothstep(0.05, 0.50, lum);\n"
    "    vec3 result = mix(col, paper, paperMix);\n"
    "    \n"
    "    float darkLift = smoothstep(0.0, 0.15, lum);\n"
    "    result = mix(paper * 0.95, result, darkLift);\n"
    "    \n"
    "    result = mix(result, inkColor, inkMask * 0.20);\n"
    "    \n"
    "    float h1 = step(0.5, fract((uv.x + uv.y) * 36.0));\n"
    "    float h2 = step(0.5, fract((uv.x - uv.y) * 36.0));\n"
    "    float hatch = 0.0;\n"
    "    if (lum > 0.28 && lum < 0.88) hatch = max(h1 * 0.50, h2 * 0.35);\n"
    "    result = mix(result, inkColor * 0.85, hatch * 0.06);\n"
    "    \n"
