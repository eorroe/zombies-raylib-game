...
static const char *postFragShader = 
    ...
    "    float grain = (fract(sin(dot(uv, vec2(12.9898, 78.233))) * 43758.5453) - 0.5) * 0.12;\n"
    "    col += grain;\n"
    "    float bloom = smoothstep(0.5, 1.0, max(col.r, max(col.g, col.b))) * 0.6;\n"
    "    col += bloom;\n"
    "    float fog = smoothstep(0.2, 1.0, length(uv - 0.5) * 1.4);\n"
    "    col = mix(col, vec3(0.2, 0.1, 0.3), fog * 0.6);\n"
    "    col.r *= 1.1;\n"
    "    col.b *= 0.9;\n"
