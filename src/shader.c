...
static const char *postFragShader = 
    ...
    "    edge = smoothstep(0.05, 0.30, edge);\n"
    "    float inkMask = floor(edge * 6.0 + 0.5) / 6.0;\n"
    "    \n"
    ...
    "    float paperMix = smoothstep(0.3, 0.8, lum) * 0.35;\n"
    "    vec3 result = mix(col, paper, paperMix);\n"
    "    \n"
    "    result = mix(result, inkColor, inkMask * 0.95);\n"
    "    \n"
    ...
    "    result = mix(result, inkColor * 0.6, ruledLine * 0.25);\n"
    "    \n"
