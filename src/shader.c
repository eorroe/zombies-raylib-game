...
static const char *postFragShader = 
    ...
    "    float paperMix = smoothstep(0.15, 0.85, lum) * 0.80;\n"
    ...
    "    \n"
    "    float h1 = step(0.5, fract((uv.x + uv.y) * 40.0));\n"
    "    float h2 = step(0.5, fract((uv.x - uv.y) * 40.0));\n"
    "    float hatch = 0.0;\n"
    "    if (lum > 0.30 && lum < 0.90) hatch = max(h1 * 0.95, h2 * 0.8);\n"
    "    result = mix(result, inkColor * 1.0, hatch * 0.50);\n"
    "    \n"
