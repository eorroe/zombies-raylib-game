...
static float TorsoRadius(float ny, float theta, float width, float height, float depth) {
    ...
    float shoulder = 1.0f + 0.3f * powf(fmaxf(0.0f, ny - 0.3f), 2.0f);
    float waist = 1.0f - 0.1f * powf(fmaxf(0.0f, -ny), 3.0f);
    float angleMod = 1.0f;
    angleMod += 0.02f * sinf(theta * 6.0f + ny * 4.0f);
    angleMod += 0.015f * sinf(theta * 10.0f - ny * 3.0f);
...
Mesh SoldierMesh_CreateTorso(float width, float height, float depth) {
    ...
    SoldierMesh_ComputeNormals(&mesh);
    return mesh;
}
...
Mesh SoldierMesh_CreateHead(float radius) {
    ...
    for (int j = 0; j <= heightSegs; j++) {
        ...
        for (int i = 0; i <= radialSegs; i++) {
            ...
            mesh.texcoords[vi * 2 + 0] = u;
            mesh.texcoords[vi * 2 + 1] = v;
            mesh.colors[vi * 4 + 0] = 80;
            mesh.colors[vi * 4 + 1] = 120;
...
Mesh SoldierMesh_CreateHelmet(float radius) {
    ...
    for (int j = 0; j <= heightSegs; j++) {
        ...
        for (int i = 0; i <= radialSegs; i++) {
            ...
            mesh.texcoords[vi * 2 + 0] = u;
            mesh.texcoords[vi * 2 + 1] = v;
            mesh.colors[vi * 4 + 0] = 20;
            mesh.colors[vi * 4 + 1] = 30;
